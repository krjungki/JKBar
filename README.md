...
- Bar 왼쪽에는 내장 4분할 기본 이미지가 표시된다. 사용자가 고른 이미지는 기본 이미지보다 우선하며,
  설정에서 `기본 이미지`를 누르면 내장 이미지로 돌아간다.
- 기본 또는 사용자 이미지를 클릭하면 Windows 시스템 정보, 컴퓨터 재시작과 종료 메뉴가 열린다. 재시작과
  종료는 저장하지 않은 작업 손실 경고에서 다시 확인해야 실행된다. 메뉴 밖을 클릭하면 메뉴가 닫힌다.
- 겹침 처리를 세 방식 중에서 고른다. 최상위 표시 / 상단 영역 예약 / 바탕화면 고정.
- 표시 모니터의 기본값은 `자동 (Windows 주 모니터)`이며 Windows의 현재 Primary 화면을 따라간다. 명시적으로
  고른 화면이 분리되면 Primary 화면으로 즉시 이동하고 설정도 자동 모드로 바꾼다.
...
- 사용자가 지정한 RSS/Atom 피드의 최신 제목과 현지 시각으로 바꾼 포스팅 시각을 왼쪽 띠에 표시하고,
  클릭하면 원문을 연다. 배포처가 URL의 `&`를 escape하지 않은 잘못된 XML을 보내면 해당 문자만 안전하게
  보정해 한 번 더 파싱한다.
- 뉴스 표시 여부, 피드 주소, 갱신 주기와 기사 전환 주기를 로컬 설정에 저장한다.
- 뉴스와 시스템 지표를 단색 Bold로 표시하며 글꼴, 스타일, 크기와 글자색을 설정해 저장한다.
- Bar와 노치 글자는 DirectWrite가 CPU에서 모양과 회색조 가장자리를 계산해 GDI+ 표면에 합성한다. Direct2D나
  Direct3D 장치를 만들지 않아 하이브리드 그래픽 전환을 방해하지 않는다.
- CPU·GPU·RAM은 이름과 숫자, 세로 이름과 그래프, 세로 이름·숫자·그래프 중에서 표시 형식을 고른다.
  그래프 색은 글자색을 따르거나 따로 지정한다.
- 가독성을 위해 글자 그림자를 켜고 끕 수 있다.
- CPU·GPU·RAM·DISK·NET·시계와 GSA·OneDrive·Syncthing 상태를 개별적으로 켜고 끄거나 순서를 바꾸며 설정을
  저장한다. 최신 상태는 초록 체크, 동기화 중은 노란 회전 화살표, 오류·불명 상태는 빨간 느낌표로 구분한다.
  native 앱이 감지되지 않으면 해당 표시 항목은 회색 미체크로 보이며 저장된 선택은 유지된다.
- OneDrive는 현재 Windows 사용자 SID의 동기화 루트와 JKBar가 실행 중인 로그인 세션의 프로세스만 감시한다.
  다른 로컬 계정의 연결 끊긴 루트나 다른 세션의 전송 활동은 경고를 유발하지 않으며, 현재 사용자의 실제
  오류·연결 끊김은 계속 표시한다.
- Bar 설정은 `Bar 모양`과 `표시항목`으로 나뉜다. 표시항목에서 OneDrive·Global Secure Access·Syncthing의
  정상 복구와 동기화/주의 상태 변화를 확장 알림으로 표시할지 각각 정하며, 상태 아이콘 표시 여부와는 독립적이다.
  동기화 알림은 `동기화가 진행 중입니다`, 실제 오류는 `오류가 보고되었습니다`로 서로 다르게 표시한다.
  감지되지 않은 앱의 알림 설정은 회색으로 비활성화되고 저장된 선택은 유지된다.
- 설정의 `프로세스 리스트`에 이름과 실행 파일 경로를 등록하면, 그 프로그램이 실행 중일 때만 노치와 CPU 사이에
  아이콘이 나타나 실행 여부를 알려 준다. 항목별로 아이콘 위 실행 프로세스 개수 배지를 숨길 수 있으며,
  아이콘을 누르면 그 프로그램의 창을 맨 앞으로 가져온다.
- 평상시 노치는 비우기, 날짜·시간, 픽셀 동물(개·고양이·팬더·오리·햄스터) 중 선택한다. 동물은 큰 눈과 종별
  무늬를 가진 픽셀 아트로 표시되며, 걷기·달리기·웅크림·점프·착지·기지개·벽 타기·말풍선·정면 휴식을 번갈아
  보여준다. 동물 뒤에는 푸른 들판·노을 들판·고목 초원·버드나무 호수·열대 해변 배경을 고를 수 있다. 알림과
  재생 정보가 우선한다. 설정 → 기본 → 평상시 노치에서 바꿀 수 있다.
- 트레이 아이콘의 좌·우 클릭 메뉴에서 현재 버전을 확인하고 통합 설정 창을 열거나 앱을 종료한다.
- 노치를 오른쪽 클릭하면 트레이와 같은 메뉴에서 설정을 열거나 업데이트를 확인하고 앱을 종료할 수 있다.
  두 메뉴 모두 다른 창이나 메뉴 밖 위치를 클릭하면 닫힌다.
- 뉴스 제목과 실행 앱 아이콘만 왼쪽 클릭을 받으며, 빈 띠와 투명한 노치 모서리는 클릭을 통과시킨다.
...
이미지 경로, 피드, 글꼴, 표시 항목, 내장 앱 확장 알림 선택, 표시할 모니터, 등록한 종목과 평상시 노치 설정은 실행 파일 옆
...
사용자 이미지를 고르면 원본을 계속 참조하지 않는다. 내장 기본 이미지는 실행 파일 안에 있으며, 사용자 이미지는
`settings.json`이 있는 폴더에
